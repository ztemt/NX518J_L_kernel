...
static int pressure_host_status_report(struct pressure_sensor_data *pdata, enum HOST_STATUS status)
{
		...
		do {
			...
			if (err >= 0) {
				SENSOR_LOG_INFO("send i2c ctrl slave status cmd ok\n");
				break;
			}
			msleep(1);
...
static ssize_t pressure_poweroff_host_status_store(struct device *dev,
				struct device_attribute *attr,
				const char *buf, size_t size)
{
		struct pressure_sensor_data *pdata = dev_get_drvdata(dev);
...
static struct device_attribute attrs_pressure_device[] = {
		...
		__ATTR(pressure_debug, 0666, NULL, pressure_debug_store),
		...
		__ATTR(report_host_status, 0664, pressure_slave_status_show, pressure_poweroff_host_status_store),
};
